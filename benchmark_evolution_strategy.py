...
function_names = ['translated_sphere', 'ackley', 'schaffer2d', 'rastrigin']
...
for i in range(len(functions)):
    ...
    for fig_format in fig_formats:
        plt.savefig(f'results/{function_name}/mean_fitness.{fig_format}')
    plt.figure()
    plt.plot(best_ses6)
    plt.plot(best_ses12)
    plt.plot(best_ses24)
    ...
    for fig_format in fig_formats:
        plt.savefig(f'results/{function_name}/best_fitness.{fig_format}')
    plt.show()
